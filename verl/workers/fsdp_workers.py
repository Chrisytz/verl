...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        super().__init__()
        print("actorrolloutrefworker init")

        self.device_name = get_device_name()
        print(f"ActorRolloutRefWorker determined device_name: {self.device_name}")
    ...
    def init_model(self):
        print("ACTORROLLOUTREFWORKER INITMODEL")
    ...
    def update_actor(self, data: DataProto):
        print("ACTORROLLOUTREFWORKER UPDATEACTOR")
    ...
    def generate_sequences(self, prompts: DataProto):
        print(f"ACTORROLLOUTREFWORKER GENERATESEQUENCES {prompts}")
        # Support all hardwares
...
class CriticWorker(Worker):
    def __init__(self, config):
        super().__init__()
        print("CriticWorker init")
        
        self.device_name = get_device_name()
        print(f"CriticWorker determined device_name: {self.device_name}")
        import torch.distributed
    ...
    def update_critic(self, data: DataProto):
        print("CRITICWORKER UPDATE_CRITIC")
        # Support all hardwares
        data = data.to(get_torch_device().current_device())
        if self._is_offload_param:
            load_fsdp_model_to_gpu(self.critic_module)
